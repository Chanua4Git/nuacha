...
serve(async (req) => {
  ...
  try {
    ...
    const activateResponse = await fetch(`${paypalApiBase}/v1/billing/subscriptions/${subscriptionId}/activate`, {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        reason: "User subscription activated"
      })
    })

    if (!activateResponse.ok) {
      const errorText = await activateResponse.text()
      console.error('PayPal subscription activation failed:', errorText)
      return new Response(
        JSON.stringify({ error: 'Failed to activate PayPal subscription' }),
        { 
          status: 500, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        }

...
INSERT INTO public.categories (id, name, color, group_type, family_id, created_at, updated_at, budget, sort_order)
SELECT 
  bc.id,
  bc.name,
  CASE 
...
-- Add a user_id column to categories to support user-specific categories
ALTER TABLE public.categories 
ADD COLUMN user_id uuid REFERENCES auth.users(id);

-- Update migrated budget categories to have user_id
UPDATE public.categories 
SET user_id = bc.user_id
FROM public.budget_categories bc
WHERE public.categories.id = bc.id;

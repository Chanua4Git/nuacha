-- Phase 1: Comprehensive Category Duplicate Cleanup
...
-- Unique constraint for family-level categories (name must be unique within family)
ALTER TABLE categories 
ADD CONSTRAINT unique_family_category_name 
UNIQUE (family_id, name) 
WHERE family_id IS NOT NULL AND is_budget_category = false;

-- Unique constraint for user-level budget categories
ALTER TABLE categories 
ADD CONSTRAINT unique_user_budget_category_name 
UNIQUE (user_id, name) 
...
-- Step 7: Refresh constraints and indexes

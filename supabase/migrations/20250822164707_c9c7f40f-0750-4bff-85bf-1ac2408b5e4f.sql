...
BEGIN
  ...
  LOOP
    ...
    RAISE NOTICE 'Cleaned up % user-level duplicates for category: %', duplicate_rec.cnt - 1, duplicate_rec.name;
  END LOOP;
END $$;
...
BEGIN
  ...
  LOOP
    ...
    RAISE NOTICE 'Cleaned up % family-level duplicates for category: %', duplicate_rec.cnt - 1, duplicate_rec.name;
  END LOOP;
END $$;
...
RAISE NOTICE 'Category duplicate cleanup completed successfully';